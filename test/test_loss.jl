...
function test_loss(lf::LossFunction, 
  ...
  for i = 1:size(y_examples, 1)
    @test loss(lf, y_examples[i], y_pred_examples[i]) ≈ expected[i]
  end
end
...
function test_minimizing_scalar(lf::LossFunction, 
  ...
  for i = 1:size(y_examples, 1)
    @test minimizing_scalar(lf, y_examples[i]) ≈ expected[i]
  end
end
...
@testset "Loss functions" begin
  @testset "not implemented functions throw an error" begin
    ...
    @test_throws UndefVarError loss(dlf, emp_vec, emp_vec)
    @test_throws UndefVarError negative_gradient(dlf, emp_vec, emp_vec)
    @test_throws UndefVarError minimizing_scalar(dlf, emp_vec)
  end
  ...
  @testset "BinomialDeviance loss works" begin
    lf = BinomialDeviance()
    expected = [ 
      0.6265233750364456, 1.6265233750364456, 1.6265233750364456, 2.6265233750364456, 2.6265233750364456,
    ]
